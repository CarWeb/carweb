<html>
		...
		<div id="All">
			...
			<div id="main">	
				<div class="whole">	
					...
					<form id="form1" action="/maven02/login/selectByName" method="get" class="form_box">
	...
	<script>
						...
						if(data == 0){
							...
							/////////////////////////登录成功/////////////////////////////////////////
								...
								if(data.admin == 1){
									...
									html += '</div>';
									$("#return").empty().append(html);
									setTimeout("ToAdminlogin()",999);//管理员跳后台"+data.id+"
								}
									...
									html += '</div>';
									$("#return").empty().append(html);
									setTimeout("ToUserlogin("+data.id+")",999);//普通用户跳前台
								}
								else{
									alert("error");
								}
			...
			function ToUserlogin(id){
				window.location.href="/maven02/Hcar/user?id="+id+"";
			}
			function ToAdminlogin(){
				window.location.href="/maven02/car/admin#";
			}
